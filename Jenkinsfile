    ...
    stage('Load GPG Key for Signing') {
      steps {
        sh '''
          ...
          echo $GIT_COMMIT
          gpg --no-default-keyring --keyring=~/.gpg/$GIT_COMMIT.gpg --batch --import $GPG_SECRET
          gpg --no-default-keyring --keyring=~/.gpg/$GIT_COMMIT.gpg --import-ownertrust $GPG_OWNERTRUST
          ##gpg --list-keys
